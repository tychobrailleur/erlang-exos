...
-export([init/0, setup_schema/0, add_user/3, get_user/1, list_users/0, add_tip/3, get_tip/1,
         list_tips/0, get_abuse/1, add_abuse/2, list_abuse/0, delete_tables/0, count_recent_tips/1,
         count_different_ips/1]).
-include_lib("stdlib/include/qlc.hrl").

-define(SALT_LENGTH, 12).
-define(SECONDS_IN_DAY, 24*3600*60).
...
add_tip(Url, Description, User) ->
    ...
    RecentTips = count_recent_tips(User),
    case RecentTips > 10 of
        true -> { error, too_many_tips };
        false -> DifferentIps = count_different_ips(User),
                 case DifferentIps > 3 of
                     true -> { error, too_many_different_ips };
                     false -> do_add_tip(Url, Description, User)
                 end
    end.

do_add_tip(Url, Description, User) ->
...
count_recent_tips(User) ->
    Tips = do(qlc:q([ T || T <- mnesia:table(tips), T#tip.user =:= User, calendar:datetime_to_gregorian_seconds(erlang:localtime()) - calendar:datetime_to_gregorian_seconds(T#tip.review_date) < ?SECONDS_IN_DAY ])),
    length(Tips).

count_different_ips(User) ->
    Abuse = do(qlc:q([ A#abuse.ip || A <- mnesia:table(abuse), A#abuse.user =:= User, calendar:datetime_to_gregorian_seconds(erlang:localtime()) - calendar:datetime_to_gregorian_seconds(A#abuse.timestamp) < (?SECONDS_IN_DAY*3) ], {unique, true})),
    length(Abuse).
